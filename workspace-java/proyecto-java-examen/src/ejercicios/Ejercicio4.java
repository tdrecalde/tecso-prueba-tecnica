...
public class Ejercicio4 {
	...
	private static void informacion(List<Integer> lista1, Integer numero) {
		...
		/* Recorre una lista de enteros calculando el mod de cada valor para verificar 
		   si son pares y los cuenta si cumplen esta condición.
		 */
		int pares = 0;
		 ...
		 * y si cumple esta condición, lo agrega a una segunda lista, para luego imprimirlos.
		*/
		 ...
		 * despúes con el resultado imprimir el contenido que se encuentre en el índice de la lista, .
		 * */
		int p = lista1.size() / 2;
		 ...
		 * y cuenta cada caso que cumpla esta condición, luego imprime si más de la mitad de 
	...
	private static List<Integer> unionListas(List<Integer> lista1, List<Integer> lista2) {
		...
		List<Integer> retList = new ArrayList<Integer>(lista1);
		
		retList.addAll(lista2);
		
		for (Integer m: retList) {
			if (!union.contains(m)) {
				union.add(m);
			}
		}
	...
	private static List<Integer> interseccionListas(List<Integer> lista1, List<Integer> lista2) {
		...
		retList.retainAll(lista2);
		Set<Integer> retSet = new TreeSet<Integer>(retList);
		retSet.retainAll(lista1);
		
		return  new ArrayList<Integer>(retSet);
	...
	private static List<Integer> ordenaListaDescendente(List<Integer> lista2) {
		List<Integer> retList = new ArrayList<Integer>(lista2);
		retList.sort(Comparator.reverseOrder());
		return retList;
	}
